...
import sentiment from "../../services/sentiment.service"
...
const Transactions = props => {
  ...
  // async function getWatchlistByUser() {
  //   if (localStorage.getItem("user")) {
  //     setIsLogin(true);
  //     const obj = JSON.parse(localStorage.getItem("user"));
  //     const response = await watchlist.getWatchlistbyUser(obj.id);
  //     if (response.status == 200 && response.data.watchlistResponse!== undefined && response.data.watchlistResponse.length > 0) {
  //       setwatchlistData(response.data.watchlistResponse);
  //       //getSymboleQuote(response.data.watchlistResponse);
  //     }
  //     else
  //     {
  //       setwatchlistData([]);
  ...
  //     if (
  //       response.status == 200 &&
  //       response.data.symbolPostResponse !== undefined &&
  //       response.data.symbolPostResponse.length > 0
  //     ) {
  ...
  async function getKudosCoinAwarded() {
    if (localStorage.getItem("user")) {
      ...
      //   }else if ( response.status == 200 && esponse.data.kudosCoinResponse === null) {
   ...
   async function getSymboleQuote(response){
    for (const element of response) {
      ...
      axiosHttpMiddelware.post("/symbolQuoteSummary",{symboleData}).then((quouteresponse) => {
        if (quouteresponse.status == 200 && quouteresponse.data.symbolResponse!== undefined && quouteresponse.data.symbolResponse.length > 0)
        {
          setwatchlistData({"symbolename":element.symbolename,"currentprice":quouteresponse.data.symbolResponse[0].financialData.currentPrice.fmt});
        }
        else{
  ...
  async function getNumerofPostbyTagFilter(filterdata) {
    if (localStorage.getItem("user")) {
      setnumberofpostData([])
  ...
  async function getNumerofPostbyTimeFilter(filterdata) {
    if (localStorage.getItem("user")) {
      setIsLogin(true)
      setnumberofpostData([])
  ...
  async function getKudosCountByTimeFilter(filterdata) {
    if (localStorage.getItem("user")) {
      ...
      setkudoscoinData([])
      // axiosHttpMiddelware.post("/getKudosCountByTimeFilter",{filterdata}).then((response) => {
      //   if (
      //     response !== undefined &&
      //     response.data.filterResponse !== undefined &&
      //     response.status == 200 &&
  ...
  async function getSentimentCountByTagFilter(filterdata) {
    if (localStorage.getItem("user")) {
      ...
      axiosHttpMiddelware.post("/getSentimentCountByTimeFilter",{filterdata}).then((response) => {
        if (
          response !== undefined &&
          response.data.filterResponse !== undefined &&
          response.status == 200 &&
                    ...
                    <div className="table-responsive pt-1 text-center">
                      <Table className="table align-middle table-nowrap">
                        <tbody>
                          ...
                          {kudoscoinData === null ? <h4> No data </h4> : <></>}
